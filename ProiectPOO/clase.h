...
public:
	...
	//INSERT INTO studenti VALUES (Delia, 20, 6), (Ovidiu, 21, 7.5) 
...
parantezaStanga++;
	...
	//DELETE FROM STUDENTI WHERE NUME = DELIA
	...
	{
		//delete from studenti where nume = delia
		//select all from studenti where nume = delia
		char* cuvant;
		char* copie;
		copie = new char[comanda.length() + 1];
		strcpy(copie, comanda.c_str());
	...
	void update(string comanda, Tabela& t)//update studenti set varsta = 22 where nume = delia
	{
		char* cuvant;
		char* copie;
		string aux;
		...
		if (gasit)
		{
			...
			char inlocuimCu[255] = "";//update studenti set varsta = 22 where nume = delia
...
public:
	...
	void findKeyWord(string comanda)
	{
		...
		else  if (strcmp(cuvant, "SELECT") == 0)
		{
			...
			else //select nume, varsta from studenti where nume = delia
			{
				if (strcmp(cuvant, "FROM") == 0)
				{
					throw exception("Comanda invalida !");
		...
		else  if (strcmp(cuvant, "UPDATE") == 0) //update studenti set varsta = 22 where nume = delia
		{
			cuvant = strtok(NULL, " ");
			if (cuvant == nullptr)
			{
